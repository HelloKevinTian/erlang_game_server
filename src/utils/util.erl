...
get_flytime(SkillSpeed,PosMy,PosEnemy)->
	...
	if
		SkillSpeed =< 0->
			0;
		Myx=:=Enemyx->
			erlang:trunc(erlang:abs(Myy - Enemyy)*?Y_PIXELS/SkillSpeed*?FRAME_TIME);
		Myy=:=Enemyy->
			erlang:trunc(erlang:abs(Myx - Enemyx)*?X_PIXELS/SkillSpeed*?FRAME_TIME);
		true->
			if
...
get_abs_distance({X1,Y1},{X3,Y3})->
	math:sqrt(math:pow(Y3-Y1, 2)+math:pow(X3-X1, 2)).
...
json_encode({struct,_MemberList}=Term)->
	try
		...
		E:R-> 
			slogger:msg("json_encode exception ~p:~p",[E,R]),
			{error,"Excption!"}
	end;
json_encode({scribe_valuetolist,_MemberList}=Term)->
	try
		...
		E:R-> 
			slogger:msg("json_encode scribe_valuetolist exception ~p:~p",[E,R]),
			{error,"Excption!"}
	end;
json_encode({scribe_static,_MemberList}=Term)->
	try
		...
		E:R-> 
			slogger:msg("json_encode scribe_static exception ~p:~p ~p",[E,R,Term]),
			{error,"Excption!"}
	end;
...
get_server_start_time()->
	try
		case env:get(server_start_time, []) of
			[]->
				[];
		...
		E:R->
			slogger:msg("get_server_start_time errorE:~p,R:~p~n",[E,R]),
			[]
	end.
...
sprintf(Format,Data)->
	try
		...
		E:R->
			slogger:msg("util:sprintf error E:~p,R:~p~n",[E,R])
	end.

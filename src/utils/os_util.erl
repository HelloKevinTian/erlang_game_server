...
stop_evm(OptionList)->
	...
	case file:consult(NewOptFile) of
		{error,Reason}->
			slogger:msg("Error:~p [~p]~n",[Reason,NewOptFile]);
		{ok,[RunOptions]}->
			{_,NodesOption} = lists:keyfind(nodes, 1, RunOptions),
			Cookie = get_running_cookie(),
			erlang:set_cookie(node(), Cookie),
			os_wait(2),
			PreFix  = run_option:get_opt_prefix(RunOptions),
			...
			case Type of
				...
				cache->
					case run_option:get_opt_cache_nodes(RunOptions) of
						[]->
							slogger:msg("get control node option error ~n");
						CacheNodesOption->
							lists:foreach(fun(RunOpt)-> 
												  {SNode,Ip,_,_,_} = RunOpt,
												  case match_ip(Ip) of
...
stop_special_node(RunOptions, PreFix) ->
	case run_option:get_opt_control_nodes(RunOptions) of
		[]->
			slogger:msg("get control node option error ~n");
		ControlNodesOption->
			lists:foreach(fun(RunOpt)-> 

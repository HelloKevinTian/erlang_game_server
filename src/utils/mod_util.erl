...
behaviour_apply(Behaviour,Func,Args)->
	lists:foreach(fun(Mod)->
						  case Behaviour of
							  cache_operater_mod->
								  {Time,_Value}=timer:tc(mod_util,safe_aplly,[Mod, Func, Args]),
								  slogger:msg("cache_operater_mod Mod:~p Func:~p Time:~p ~n",[Mod,Func,{Time/1000}]);
							  _->
...
safe_aplly(Mod, Func, Args)->
	try
		...
		E:R->
			slogger:msg("~p:~p ~p ~p ~p ~p~n",[Mod, Func, Args,E,R,erlang:get_stacktrace()])
	end.
